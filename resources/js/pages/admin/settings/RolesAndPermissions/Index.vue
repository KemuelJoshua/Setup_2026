...
import { destroy, index, store, update } from '@/routes/admin/settings/roles';
...
<template>
    ...
    <div class="flex flex-1 flex-col gap-5">
        <TableToolbar v-model="searchQuery" title="Roles & permissions"
            description="Manage access levels and the guards assigned to each role." :count="roles.total"
            item-label="role" search-placeholder="Search roles…" search-label="Search roles">
            <template #filters>
                <label class="relative block sm:w-36">
                    <span class="sr-only">Filter roles by guard</span>
                    <select v-model="selectedGuard"
                        ...
                        <option v-for="guard in ['all', ...guards]" :key="guard" :value="guard">
            ...
            <template #actions>
                <Sheet v-model:open="isCreateSheetOpen">
                    ...
                    <SheetContent side="right" class="w-full gap-0 p-0 sm:max-w-lg">
                        <Form v-bind="store.form()" v-slot="{ errors, processing }" reset-on-success
                            ...
                            <div class="flex-1 space-y-6 overflow-y-auto px-6 py-5">
                                <div class="grid gap-2">
                                    ...
                                    <Input id="role-name" name="name" placeholder="e.g. Instructor" autocomplete="off"
                                        autofocus />
                                ...
                                <fieldset class="grid gap-3">
                                    ...
                                    " class="relative">
                                        ...
                                        <Input v-model="permissionSearchQuery" type="search"
                                            aria-label="Search permissions" placeholder="Search permissions…"
                                    ...
                                    <div v-if="
                                        availablePermissions.length > 0
                                    " class="grid gap-5">
                                        <section v-for="group in categorizedPermissions" v-show="group.permissions.some(
                                            matchesPermissionSearch,
                                        )
                                            " :key="group.category" class="grid gap-2">
                                            <h3
                                                class="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                                                {{ group.category }}
                                            </h3>
                                            <div class="grid gap-2 sm:grid-cols-2">
                                                <Label v-for="permission in group.permissions" v-show="matchesPermissionSearch(
                                                    permission,
                                                )
                                                    " :key="permission.id" :for="`permission-${permission.id}`"
                                                    class="cursor-pointer rounded-lg border border-border p-3 transition-colors hover:bg-muted/50">
                                                    <Checkbox :id="`permission-${permission.id}`" name="permissions[]"
                                                        :value="permission.id
                                                            " />
        ...
        <Sheet v-model:open="isEditSheetOpen">
            <SheetContent side="right" class="w-full gap-0 p-0 sm:max-w-lg">
                <Form v-if="selectedRole" :key="selectedRole.id" v-bind="update.form(selectedRole.id)"
                    v-slot="{ errors, processing }" class="flex min-h-0 flex-1 flex-col"
                    :options="{ preserveScroll: true }" @success="closeEditSheet">
                    <SheetHeader class="border-b border-border px-6 py-5 text-left">
                    ...
                    <div class="flex-1 space-y-6 overflow-y-auto px-6 py-5">
                        <div class="grid gap-2">
                            ...
                            <Input id="edit-role-name" name="name" :default-value="selectedRole.name" autocomplete="off"
                                autofocus />
                            <InputError :message="errors.name" />
                        </div>
                        ...
                        <fieldset class="grid gap-3">
                            ...
                            <div v-if="availablePermissions.length > 0" class="relative">
                                <Search aria-hidden="true"
                                    class="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" />
                                <Input v-model="permissionSearchQuery" type="search" aria-label="Search permissions"
                                    placeholder="Search permissions…" autocomplete="off" class="pl-9" />
                            </div>
                            ...
                            <div v-if="availablePermissions.length > 0" class="grid gap-5">
                                <section v-for="group in categorizedPermissions" v-show="group.permissions.some(
                                    ...
                                    <div class="grid gap-2 sm:grid-cols-2">
                                        <Label v-for="permission in group.permissions" v-show="matchesPermissionSearch(
                                            ...
                                            " :key="permission.id" :for="`edit-permission-${permission.id}`"
                                            class="cursor-pointer rounded-lg border border-border p-3 transition-colors hover:bg-muted/50">
                                            <Checkbox :id="`edit-permission-${permission.id}`" name="permissions[]"
                                                :value="permission.id" :default-value="selectedRole.permission_ids.includes(
                                                    permission.id,
                                                )
                                                    " />
                                            <span class="min-w-0 break-words">
                                ...
                                <p v-if="matchingPermissionCount === 0"
                                    class="rounded-lg border border-dashed border-border px-4 py-6 text-center text-sm text-muted-foreground">
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-xl text-left text-sm">
                ...
                <tbody class="divide-y divide-border/60">
                    <tr v-for="role in roles.data" :key="role.id" class="transition-colors hover:bg-muted/35">
                        <td class="px-2 py-3">
                            <div class="flex items-center gap-3">
                                <span
                                    class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-primary/8 text-primary dark:bg-primary/15">
                                    <ShieldCheck aria-hidden="true" class="size-4.5" />
                                </span>
                        ...
                        <td class="px-2 py-3">
                            <div class="flex justify-end gap-2">
                                <Button type="button" variant="outline" size="sm" :aria-label="`Edit ${role.name}`"
                                    @click="openEditSheet(role)">
                                    <Pencil aria-hidden="true" />
                                </Button>
                                <Button type="button" variant="destructive" size="sm"
                                    :aria-label="`Delete ${role.name}`" @click="openDeleteDialog(role)">
        ...
        <TablePagination :from="roles.from" :to="roles.to" :total="roles.total" :links="roles.links"
            :previous-page-url="roles.prev_page_url" :next-page-url="roles.next_page_url" />
    </div>

    <Dialog v-model:open="isDeleteDialogOpen">
        <DialogContent v-if="rolePendingDeletion">
            <Form v-bind="destroy.form(rolePendingDeletion.id)" v-slot="{ processing }" class="space-y-6"
                :options="{ preserveScroll: true }" @success="closeDeleteDialog">
